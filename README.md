...
학습한 컴퓨터 비전의 다양한 이미지 전처리 기법을 유기적으로 결합하여 인식률을 개선하였으며, 수집된 데이터는 결측치 검수 로직을 거쳐 SQLite를 통해 데이터베이스에 자동 적재되며, 추후 캐릭터 스테이터스 변화에 따른 데미지 편차를 다중 선형 회귀 분석(데이터 분석 프로젝트)하기 위한 파이프라인의 기초 자료로 활용됩니다.
